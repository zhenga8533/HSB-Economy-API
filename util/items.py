...
def parse_auction(items: dict, auction: dict) -> None:
    ...
    lbin = float("inf") if current is None else current.get("lbin")
    timestamp = (
        auction.get("timestamp", datetime.now().timestamp()) / 1000
        if current is None or within_percent(item_bin, lbin, 5) or item_bin < lbin
        else current.get("timestamp")
    )
    item = {"lbin": item_bin if current is None else min(item_bin, current.get("lbin")), "timestamp": timestamp}
    ...
    if attributes is not None:
        ...
        if is_kuudra_piece:
            ...
            if check_combo and len(attribute_keys) > 1:
                ...
                elif within_percent(current_cost, item_bin, 5):
